...
any columns that contain "dso", "dental service organization", "service org",
"dental support organization", "support organization", "support org", "service", or "support" in their names.
"""
import logging
...
def scan_sql_file(filepath):
    ...
    for table_match in table_regex.finditer(content):
        ...
        # Only include tables with matching column names: dso, dental service organization, service org, dental support organization,
        # support organization, support org, service, or support.
        col_match = re.search(r'\b(?:dental service organization|dental support organization|service org|support organization|support org|dso|service|support)\b', columns_section, flags=re.IGNORECASE)
        if col_match:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scan SQL files and report database table info for DSO-related column information")
